...
export function EngineRoomPane({ sendToolInput }: EngineRoomPaneProps) {
    ...
    toolEvents.forEach((event) => {
      if (
        ...
        (event.terminalOutput &&
          BLOCKING_PROMPT_REGEX.test(event.terminalOutput))
      ) {
        const now = Date.now();
  ...
  const handleSendInput = async (e?: React.FormEvent) => {
    ...
    const activeTool = [...toolEvents]
      .reverse()
      .find((e) => e.status === 'running' || e.status === 'awaiting_input');
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isFlashing &&
          'ring-4 ring-yellow-500 shadow-[0_0_30px_rgba(234,179,8,0.5)]',
      )}
    >
      {/* Header */}
    ...
    <div className="border border-border rounded-lg overflow-hidden bg-card">
      ...
      <div className="px-4 py-2 bg-muted/50 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {event.inputArguments &&
            Object.keys(event.inputArguments).length > 0 && (
              <span className="text-xs text-muted-foreground">
                {JSON.stringify(event.inputArguments)}
              </span>
            )}
        </div>
        <div className="flex items-center gap-4">
          <div className="text-[10px] text-muted-foreground flex flex-col items-end leading-none">
            ...
            {event.completedAt && (
              <span>
                {((event.completedAt - event.startedAt) / 1000).toFixed(1)}s
                duration
              </span>
            )}
          </div>
