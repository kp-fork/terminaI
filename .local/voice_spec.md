...
- **TTS-only** spoken replies are implemented (auto TTS providers + word cap).
- Push-to-talk capture and STT transcription are **not yet wired** into the CLI.
- Voice settings and CLI flags exist, but `voice.stt.provider` is reserved for
  future STT support.
