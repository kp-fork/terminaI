...
- Process orchestration is implemented via `process_manager`
  (`packages/core/src/tools/process-manager.ts`) with manual verification steps
  in `docs/termai-process-manager.md`.
- Workspace-safe file organization is implemented via `file_ops`
...
- Distribution wrapper exists: `packages/termai` (bundles `system.md` and
  launches `@google/gemini-cli`), plus `scripts/termai-install.sh`.
...
- Restrictive defaults: avoid “spawn arbitrary binary” without explicit user
  intent/confirmation.
  - Status: **implemented** as `agent_control`.
...
- Interruptible TTS; short replies by default (~<30 words).
  - Status: **TTS-only** spoken replies implemented; PTT/STT still pending.
