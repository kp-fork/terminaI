...
- **Workspace settings**: `your-project/.terminai/settings.json`
...
| Hide Context Summary           | `ui.hideContextSummary`                  | Hide the context summary (terminaI.md, MCP servers) above the input. | `false` |
...
| Load Memory From Include Directories | `context.loadMemoryFromIncludeDirectories`        | Controls how /memory refresh loads terminaI.md files. When true, include directories are scanned; when false, only the current directory is used. | `false` |
