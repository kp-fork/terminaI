...
- **[Memory Import Processor](./memport.md):** Documentation for the modular
  terminaI.md import feature using @file.md syntax.
...
- **API key management:** It handles the `TERMINAI_API_KEY` and ensures it's
  used securely when communicating with the Gemini API.
...
`terminaI.md` files that provide context to the model. It searches for these
...
`refresh` the content of loaded `terminaI.md` files.
