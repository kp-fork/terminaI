...
terminaI can integrate with your IDE to provide a more seamless and
...
- **VS Code commands:** You can access terminaI features directly from the VS
  ...
  - `terminaI: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `terminaI: View Third-Party Notices`: Displays the third-party notices for
    the extension.
...
When you run terminaI inside a supported editor, it will automatically detect
...
manually, you can run the following command inside terminaI:
...
> NOTE: The "terminaI Companion" extension may appear towards the bottom of
...
When enabled, terminaI will automatically attempt to connect to the IDE
companion extension.
...
- Open the Command Palette and run **terminaI: Accept Diff**.
...
- Open the Command Palette and run **terminaI: Close Diff Editor**.
...
If you are using terminaI within a sandbox, please be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker container:** If you run terminaI inside a Docker (or Podman)
...
- **Message:**
  `🔴 Disconnected: Directory mismatch. terminaI is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  ...
  - **Solution:** Open your IDE's extension marketplace, search for "terminaI
    Companion", and
